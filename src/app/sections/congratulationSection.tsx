...
const CongratulationSection: React.FC<CongratulationSectionProps> = ({
    ...
    const handleScoreSubmit = async () => {
        ...
        if (response) {
          alert("Successful score submission")
            setHighScores(response)
        }
        else{
    ...
    return (
        <div className="flex flex-col items-center sm:flex-row sm:justify-around px-10">
            <div className="flex flex-col sm:mr-10">
                ...
                <div className="flex flex-col items-start mt-4 mb-5 space-y-3">
                    {['Verb', 'Scarborough', 'Noun'].map((label, index) => {
                        const stateSetters = [
                            setPlayerVerb,
                            setPlayerScarborough,
                            setPlayerNoun,
                        ]
                        ...
                        return (
                            <div
                                key={label}
                                className="flex flex-row items-center justify-start space-x-2 w-full"
                            >
                    ...
                    <MainButton onClick={handleScoreSubmit}>
                        Submit Score
                    </MainButton>
                    <div className="mt-5 text-center text-sm">
                        If your score is in the top 3 so far of the day, it will
