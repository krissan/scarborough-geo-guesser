import {
  ...
  GraphQLResponse,
  HostGameForPlayerResponse,
  PlayerFinalResultResponse,
...
const getPlayer = async (gameId: string, createDate: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{ getPlayer: PlayerResponse }> =
      await response.json();
...
const getPlayers = async () => {
  ...
  try {
    ...
    const result: GraphQLResponse<{ getPlayers: PlayerResponse[] }> =
      await response.json();
...
const getPlayerCount = async (gameId: string, createDate: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{ getPlayerCount: number }> =
      await response.json();
...
const removePlayer = async (gameId: string, playerId: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{ removePlayer: boolean }> =
      await response.json();
...
const setPlayer = async (playerName: string, gameId: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{ setPlayer: PlayerResponse }> =
      await response.json();
...
const checkQuestionAnswered = async (questionId: string, playerId: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{ checkQuestionAnswered: boolean }> =
      await response.json();
...
const getHostGameForPlayer = async () => {
  ...
  try {
    ...
    const result: GraphQLResponse<{
      getHostGameForPlayer: HostGameForPlayerResponse;
    }> = await response.json();
...
const getPlayerFinalResult = async (playerId: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{
      getPlayerResult: PlayerFinalResultResponse;
    }> = await response.json();
...
const getCurrentQuestionOptions = async (
  gameId: string,
  questionId: string
) => {
  const query = `
  ...
  try {
    ...
    const result: GraphQLResponse<{ getCurrentQuestionOptions: string[] }> =
      await response.json();
...
const setScore = async (
  ...
  try {
    ...
    const result: GraphQLResponse<{ setScore: SetScoreResponse }> =
      await response.json();
...
const getCurrentQuestionImage = async (gameId: string, createDate: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{
      getCurrentQuestionImage: QuestionImageResponse;
    }> = await response.json();
...
const checkPlayerExists = async (playerId: string, gameId: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{ checkPlayerExists: boolean }> =
      await response.json();
...
const getPlayerScores = async (gameId: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{ getPlayerScores: PlayerScoreResponse[] }> =
      await response.json();
...
const getPlayerQuestionScores = async (gameId: string, questionId: string) => {
  ...
  try {
    ...
    const result: GraphQLResponse<{
      getPlayerQuestionScores: PlayerQuestionScoreResponse[];
    }> = await response.json();
...
const getDailyHighScores = async () => {
  ...
  try {
    ...
    const result: GraphQLResponse<{
        getDailyHighScores: DailyHighScoreResponse[];
    }> = await response.json();
    console.log(result.data?.getDailyHighScores);
    if (!result.data?.getDailyHighScores) {
      console.log("No daily high score found");
    }
    const scoreResult = result.data?.getDailyHighScores.map((score) => {
      const playerNumbers = score.playerName.split(" ");
      const playerName =
        verbs[parseInt(playerNumbers[0])] +
        scarborough[parseInt(playerNumbers[1])] +
        peopleNouns[parseInt(playerNumbers[2])];
...
const addDailyHighScore = async (
  nameNumberOne: number,
  nameNumberTwo: number,
  nameNumberThree: number,
  score: number
) => {
  const query = `
  ...
  try {
    ...
    const result: GraphQLResponse<{
      addDailyHighScore: DailyHighScoreResponse[];
    }> = await response.json();
